...
@router.get('/id/{id}')
def get_user_by_id(id: str, Authorize: AuthJWT = Depends(), db: Session = Depends(get_db)):
    Authorize.jwt_required()
...
@router.patch('/id/{id}')
def edit_user_by_id(id: str, editForm: EditUserForm, Authorize: AuthJWT = Depends(), db: Session = Depends(get_db)):
    Authorize.jwt_required()
    ...
    role = db.query(Role_TM).filter(
        Role_TM.role_name == editForm.rolename
    ).first()

    if not role:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Role '{editForm.rolename}' not found!")

    user.role_id = role.id

    if editForm.password:
        user.password = bcrypt.hashpw(editForm.password.encode("UTF-8"), bcrypt.gensalt())
